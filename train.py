...
import wandb
from load_dataset import AsyncDataBuffer, CUDAPrefetcher, process_batch_on_gpu
...
def train_model(args):
    ...
    if getattr(args, "compile_model", False) and hasattr(torch, "compile"):
        try:
            ...
            model.depth_decoder = torch.compile(
                model.depth_decoder, mode="reduce-overhead"
            )
    ...
    for epoch in range(1, args.epochs + 1):
        if (
            args.finetune_after_epoch
            and epoch > args.finetune_after_epoch
            and mode == "supervised"
        ):
        ...
        for step_in_epoch in range(args.steps_per_epoch):
            ...
            for chunk_start in range(0, t, args.seq_len):
                ...
                def unflat(feat):
                    return feat.view(b, chunk_steps, *feat.shape[1:])
                ...
                for i_step, step in enumerate(range(chunk_start, chunk_end)):
                    ...
                    chunk_preds.append(
                        {k: v for k, v in out.items() if k != "next_state"}
                    )
                ...
                with torch.autocast(
                    ...
                    for k in chunk_preds[0].keys():
                        ...
                        if val is None:
                            ...
                            batched_preds[k] = torch.cat(
                                [p[k] for p in chunk_preds], dim=0
                            )
                    ...
                    for k in chunk_targets[0].keys():
                        ...
                        if val is None:
                            ...
                            batched_targets[k] = torch.cat(
                                [t_dict[k] for t_dict in chunk_targets], dim=0
                            )
                ...
                for k in loss_dict_acc:
                    loss_dict_acc[k] = loss_dict_acc[k] + loss_dict[k] * chunk_steps
                ...
                if (global_step + 1) % args.vis_interval == 0:
                    ...
                    filepath = save_visualization(
                        last_x_t, last_target, last_out, global_step + 1, last_warp
                    )
                    if args.use_wandb and filepath:
                        wandb.log(
                            {"Visualization": wandb.Image(filepath)}, step=global_step
                        )
                ...
                if global_step == args.unfreeze_step_1 and mode == "supervised":
                    ...
                    for name, param in model.segmenter.named_parameters():
                        if any(f"model.{i}." in name for i in range(16, 20)):
                            param.requires_grad = True
                ...
                if global_step % 10 == 0:
                    ...
                    def get_val(k):
                        return (
                            loss_dict_acc[k].item() / cs
                            if isinstance(loss_dict_acc[k], torch.Tensor)
                            else loss_dict_acc[k] / cs
                        )

                    print(
                        f"[{elapsed:.1f}s] E{epoch} S{global_step} [{mode[:3]}] | Tot:{tot_val:.4f} | "
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max_buffer_size", type=int, default=64, help="异步流数据缓冲池大小"
    )
    ...
    parser.add_argument(
        "--finetune_after_epoch",
        type=int,
        default=0,
        help="在第几个Epoch后开启自监督微调 (填0表示不开启)",
    )
