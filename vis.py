...
def save_visualization(
    video_t, target_t, pred_t, step, warped_img=None, output_dir="vis_outputs"
):
    ...
    cv2.putText(
        anom_img, "Anomaly", (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1
    )
    ...
    if pred_flow is not None:
        ...
        pred_flow_img = cv2.resize(
            flow_to_color(pred_flow_np.transpose(1, 2, 0)), (half_w, half_h)
        )
    else:
    ...
    gt_depth_img = cv2.resize(
        depth_to_color(gt_depth_np, d_min, d_max), (half_w, half_h)
    )
    cv2.putText(
        gt_depth_img,
        "GT Depth",
    ...
    # 4. Pred Depth
    pred_depth_img = cv2.resize(
        depth_to_color(pred_depth_np, d_min, d_max), (half_w, half_h)
    )
    cv2.putText(
        pred_depth_img,
        "Pred Depth",
    ...
    gt_flow_img = cv2.resize(
        flow_to_color(gt_flow_np.transpose(1, 2, 0)), (half_w, half_h)
    )
    cv2.putText(
        gt_flow_img,
        "GT Flow",
    ...
    if warped_img is not None:
        ...
        warp_bgr = cv2.cvtColor(
            (np.clip(warp_np, 0, 1) * 255).astype(np.uint8), cv2.COLOR_RGB2BGR
        )
