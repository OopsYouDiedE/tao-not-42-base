...
class Conv(nn.Module):
    ...
    def forward(self, x):
        ...
        # P3 feature is the most high-res semantic feature for downstream ConvGRU
        features = x[0] # P3
        
        # Project 512-dim text-aligned semantic features to 1-dim foreground objectness
        obj_foreground = self.obj_proj(scores[0]) # (B, 1, 80, 80)
        obj_foreground_o2o = self.one2one_obj_proj(scores_o2o[0]) # (B, 1, 80, 80)
        
        return {
            'features': features,
            
            # one2many dense outputs (used for computing dense loss)
            'objectness': obj_foreground,
            'boxes': boxes[0],
            'mask_coefficients': mc[0],
            
            # one2one tracked outputs (used for inference and tracking)
...
class FastSAMStyleSegmenter(nn.Module):
    def __init__(self, base=48, hidden=768):
        super().__init__()
        self.stem = Conv(3, base, s=2)
        self.stage2 = nn.Sequential(Conv(base, base * 2, s=2), C2f(base * 2, base * 2, n=2))
        self.stage3 = nn.Sequential(Conv(base * 2, base * 4, s=2), C2f(base * 4, base * 4, n=4))
        self.stage4 = nn.Sequential(Conv(base * 4, base * 8, s=2), C2f(base * 8, base * 8, n=4))
        self.stage5 = nn.Sequential(Conv(base * 8, hidden, s=2), C2f(hidden, hidden, n=2), SPPF(hidden, hidden, k=5, n=1)) # Adapted SPPF
        
        self.up_5_to_4 = nn.Sequential(Conv(hidden, base * 8, k=1), nn.Upsample(scale_factor=2.0, mode='nearest'))
        self.fuse_4 = Conv(base * 8, base * 8, k=3)
        self.up_4_to_3 = nn.Sequential(Conv(base * 8, base * 4, k=1), nn.Upsample(scale_factor=2.0, mode='nearest'))
        self.fuse_3 = Conv(base * 4, base * 4, k=3)

        self.mask_prototypes = nn.Sequential(Conv(base * 4, base * 4, k=3), Conv(base * 4, base * 2, k=3), nn.Conv2d(base * 2, 32, kernel_size=1))
