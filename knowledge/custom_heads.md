...
`EgoPoseHead` 用于在连续时间序列中估计相机自身的旋转与平移参数。

### 3.1 真正的 6D 正交化 (Gram-Schmidt SO(3))
为了保证回归的数值稳定性并避免万向节死锁，回归输出的前 6 个分量表示两个三维向量 $a_1, a_2 \in \mathbb{R}^3$，通过 **Gram-Schmidt 正交化**构造出绝对正交且行列式为 1 的标准 $3 \times 3$ 旋转矩阵 $R \in SO(3)$：
...
### 3.2 位姿图构建
输出的后 3 个分量表示平移向量 $T$。输出被严格打包为包含了真正数学旋转矩阵的字典 `{"R": R, "t": T}`，用于下游构建 SE(3)。
