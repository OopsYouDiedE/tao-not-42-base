...
* **核心内容**：
  ...
  * **EgoPoseHead**：相机 6D 位姿旋转矩阵与平移估计头部。
...
1. **扁平化结构**：所有专题知识文档直接放置在 `knowledge/` 目录下，根除多层级目录碎片化。
2. **源码级对齐**：所有技术文档中的数学公式、张量形状（Shape）及参数名称，必须与项目根目录下的实际 Python 代码（`models/`、`utils/`、`trainer.py` 等）完全对齐。
3. **中文更新规范**：根据项目活动规则要求，整个知识库的所有专题文档均采用专业严谨的中文进行更新与维护。
