...
### 10. 首步训练耗时过长与光流偶发 inf/NaN 的深层数学奇点修复
*   **首步耗时定位**：在最初的几十个步骤中（从数据拉取完成到第 10 步耗时极长），这是因为我们在 `train.py` 中启用了 `torch.backends.cudnn.benchmark = True`。对于视频/时空模型，cuDNN 会在第一次前向传播时穷举搜索最优的 3D/2D 卷积底层算子，这会产生几分钟的一次性开销，之后的步数（如第 20、30 步）就会恢复到正常的几秒钟。这是符合预期的正常优化行为。
*   **光流 inf/NaN 奇点定位**：在第 40 步左右光流呈现 `inf`，是因为当网络随机预测的刚体平移（如自我运动 `T_cam` 或物体扭转 `dense_obj_twist`）把一些 3D 点推到了**相机背后**（即深度 $Z \le 0$）时，几何投影函数 `x / z` 遇到了物理奇点。之前代码简单地把 $Z$ clamp 到 `1e-3`，在 `float16` 混合精度下，类似于 $256 / 1e-3 = 256,000$ 的计算会瞬间撑爆 `float16` 的表达上限（`65504`），导致张量变为 `inf` 并反向传播出 `NaN` 毁掉整个网络。
*   **修复方案**：
    1. 在 `models/custom_blocks.py` 的 `project` 投影函数中，我们强制将其提升至 `float32` 进行运算，将安全 $Z$ 截断调整为更安全的 `0.01`，并对最终算出的 `uv` 坐标硬截断到 `[-50000, 50000]`，从根本上隔离了 `float16` 溢出。
    2. 在 `models/tao_core.py` 中，对最终输出的归一化光流 `flow_final_norm` 增加 `torch.clamp(..., -20.0, 20.0)` 的物理范围保护。这样即使个别像素发生极端错误投影，也不会在 L1 损失回归时产生爆炸级梯度。
